...
// ─── FIX: onTranscript is kept in a ref so recognition is only created ONCE.
// Previously, recognition was rebuilt on every render because onTranscript
// (an inline function in ChatBox) changed every render, triggering useEffect.

const VoiceInput = ({ onTranscript, disabled, lang = 'en-IN' }) => {
  const [isListening, setIsListening] = useState(false);
  const [supported, setSupported] = useState(true);
  const recognitionRef = useRef(null);
  const onTranscriptRef = useRef(onTranscript); // ← keep latest callback in ref

  // Keep ref up to date without re-creating recognition
  useEffect(() => {
    onTranscriptRef.current = onTranscript;
  }, [onTranscript]);

  // Create recognition ONCE on mount only
  useEffect(() => {
    const SpeechRecognition = window.SpeechRecognition || window.webkitSpeechRecognition;
    if (!SpeechRecognition) {
      setSupported(false);
    ...
    recognition.interimResults = false;
    recognition.lang = lang; // en-IN handles English + Hinglish well

    recognition.onstart = () => setIsListening(true);
    recognition.onend   = () => setIsListening(false);

    recognition.onerror = (event) => {
      // 'no-speech' is a normal timeout — don't alarm user
    ...
    recognition.onresult = (event) => {
      ...
      if (transcript && onTranscriptRef.current) {
        onTranscriptRef.current(transcript); // ← use ref, not stale closure
      }
    };
